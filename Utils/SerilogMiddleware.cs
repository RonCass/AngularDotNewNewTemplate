...
{
    // 3-15-2018 Ron C.: Created this to facilitate logging to the database and also inserting the users UserID in to each log entry
    ...
    public class SerilogMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            try
            {
                //3-15-2018 Ron C.: Push the User ID if they are authenticated
                if (httpContext.User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier) != null)
                {
                    var myUsersId = httpContext.User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier).Value;
                    LogContext.PushProperty("UserId", !String.IsNullOrWhiteSpace(myUsersId) ? myUsersId : "unknown");
                }
